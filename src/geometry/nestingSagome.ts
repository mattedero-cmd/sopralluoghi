 ...
 *
 * SOTTO UNA FALDA L'INGOMBRO NON BASTA. Fra due rettangoli il vuoto è per
 * forza orizzontale o verticale, e i rettangoli d'ingombro dicono la verità.
 * Sotto un lato in pendenza no: il rettangolo d'ingombro del trapezio arriva
 * fin sotto il pezzo che gli sta accosto, e la regola dell'ingombro dichiara
 * «non ci si può muovere» quando in realtà di strada ce n'è ancora — su una
 * bobina da 25 m, dove la cella della griglia arriva a quattro centimetri,
 * misurati fino a NOVE centimetri di strada buttata via, e si vedono.
 *
 * Quindi: quando il pezzo o il suo vicino non sono rettangoli si scorre sulla
 * SAGOMA VERA, cercando il punto esatto in cui le due sagome arrivano a
 * distanza di lama. Fra due rettangoli si tiene il conto sull'ingombro, che
 * dà la stessa identica risposta e non costa niente.
 *
 ...
 * cerca il minimo per sezione aurea e, se scende sotto la lama, si bisseca il
 * primo contatto. Senza la convessità un passo a tentoni potrebbe scavalcare
